...
Fix a set $B$.
\begin{itemize}
    \item $\mathcal{O}(B) = \mathcal{P}(B \times \mathcal{P}(B))$: all outcome relations on $B$.
    \item $\mathcal{O}_m(B) \subseteq \mathcal{O}(B)$: the set of monotone outcome relations.
    \item $\mathcal{G}(B)$: the set of pairs of outcome relations,
    \[
    \mathcal{G}(B) = \mathcal{O}(B) \times \mathcal{O}(B).
    \]
    \item $\mathcal{G}_m(B)$: the set of pairs of monotone outcome relations,
    \[
    \mathcal{G}_m(B) = \mathcal{O}_m(B) \times \mathcal{O}_m(B).
    \]
    \item $\mathcal{G}_{mc}(B)$: the set of consistent pairs of monotone outcome relations.
\end{itemize}
\end{definition} \\ \\ \\
Consider the following operations on $M = \mathcal{G}(B)$:
\[
(R_1, R_2) \sqcup (S_1, S_2) = (R_1 \cup S_1, \; R_2 \cup S_2) 
\quad \text{representing } g \lor h,
\]
\[
(R_1, R_2) \sqcap (S_1, S_2) = (R_1 \cap S_1, \; R_2 \cup S_2)
\quad \text{representing } g \land h,
\]
\[
(R_1, R_2)^{-} = (R_2, R_1) 
\quad \text{representing } \neg g,
\]
\[
(R_1, R_2) \square (S_1, S_2) = (R_1 \circ S_1, \; R_2 \circ S_2) 
\quad \text{representing } g \lozenge h,
\]
where
\[
R \circ S := \{ (p, T) \mid (p, U) \in R \text{ for some } U \subseteq T 
\text{ and } \forall u \in U, (u, T) \in S \}.
\]
Now define the \textbf{\emph{full outcome algebra}} on $B$ to be the structure
\[
\mathcal{O}(B) = (\mathcal{G}(B), \sqcup, \sqcap, {}^{-}, \square).
\]
Subalgebras of $\mathcal{O}(B)$ are called \textbf{\emph{outcome algebras on $B$}}.

An outcome algebra of the form $(A, \sqcup, \sqcap, {}^{-}, \square)$ 
is called \textbf{\emph{monotone}} if $A \subseteq \mathcal{G}_m(B)$ 
and a \textbf{\emph{board algebra}} if $A \subseteq \mathcal{G}_{mc}(B)$.

The \textbf{\emph{full monotone outcome algebra}} on $B$ and the \textbf{\emph{full board algebra}} on $B$ 
are defined as the structures
\[
\mathcal{G}_m(B) = (\mathcal{G}_m(B), \sqcup, \sqcap, {}^{-}, \square), 
\quad
\mathcal{G}_{mc}(B) = (\mathcal{G}_{mc}(B), \sqcup, \sqcap, {}^{-}, \square),
\]
respectively.
...
\textbf{Definition:}
Let $\mathcal{G} = (G,\vee,\wedge,\neg,\circ)$ be a game algebra. A 
\emph{module over $\mathcal{G}$} is an algebra 
$\mathcal{M} = (M,\vee,\wedge,\neg,(\Diamond_g)_{g\in G})$ 
such that $(M,\vee,\wedge,\neg)$ is a de Morgan algebra, and 
$(\Diamond_g)_{g\in G}$ is a family of unary monotone operations on $M$ 
...
$\mathcal{G} = (G,\vee,\wedge,\neg,\circ)$ as a module over its de Morgan 
reduct if we put $\Diamond_g a = g \circ a$. We will not introduce any 
...
\begin{enumerate}
    ...
    embedded in a board algebra, cf.\ Proposition 6.1.
\end{enumerate}
...
Let $\mathcap{D}$ be any distributive lattice. Define $D^b = D \cup \{\top, \bot\}$, where $\top, \bot$ are new elements. The top and bottom added are treated to be strictly new elements here. \\ \\
